...
@property (nonatomic, assign) AppAudioTypes appAudio;
@end
...
- (void)addButtonWithTitle:(NSString *)title type:(SIAlertViewButtonType)type appAudioType:(AppAudioTypes)audioType handler:(SIAlertViewHandler)handler
{
    SIAlertItem *item = [[SIAlertItem alloc] init];
	item.title = title;
	item.type = type;
	item.action = handler;
    item.appAudio = audioType;
	[self.items addObject:item];
}

- (void)show:(BOOL)playAudio
{
    ...
    if (playAudio)
        [self.appAudio play:AppAudioIfContinue withItem:nil];
...
- (void)validateLayout
{
    ...
    if (self.items.count > 0) {
        ...
        if (self.items.count == 2)
        {
            ...
            button.frame = CGRectMake(170.f, 40.f, 60.f, BUTTON_HEIGHT);
            button = self.buttons[1];
            button.frame = CGRectMake(60.f, 40.f, 60.f, BUTTON_HEIGHT);
        }
        else if (self.items.count ==3)
        {
            UIButton *button = self.buttons[0];
            button.frame = CGRectMake(204.f, 40.f, 60.f, BUTTON_HEIGHT);
            button = self.buttons[1];
            button.frame = CGRectMake(120.f, 40.f, 60.f, BUTTON_HEIGHT);
            button = self.buttons[2];
            button.frame = CGRectMake(36.f, 40.f, 60.f, BUTTON_HEIGHT);
        }
        else if (self.items.count == 4)
        {
            UIButton *button = self.buttons[0];
            button.frame = CGRectMake(120.f, 10.f, 60.f, BUTTON_HEIGHT);
            button = self.buttons[1];
            button.frame = CGRectMake(204.f, 68.f, 60.f, BUTTON_HEIGHT);
            button = self.buttons[2];
            button.frame = CGRectMake(120.f, 68.f, 60.f, BUTTON_HEIGHT);
            button = self.buttons[3];
            button.frame = CGRectMake(36.f, 68.f, 60.f, BUTTON_HEIGHT);
        }
    }
}
...
- (void)setupContainerView
{
    ...
    self.containerView.backgroundColor = [UIColor colorWithRed:23.f/255.f green:49.f/255.f blue:62.f/255.f alpha:0.85f];
//    [UIColor colorWithRed:0.f green:51/255.f blue:214.f/255.f alpha:0.75f];
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
	...
	switch (item.type) {
		case SIAlertViewButtonTypeCancel:
            normalImage = [UIImage imageNamed:@"repeat_button.png"];
			break;
		case SIAlertViewButtonTypeDefault:
			normalImage = [UIImage imageNamed:@"done_button.png"];
			break;
        case SIAlertViewButtonTypeSave:
			normalImage = [UIImage imageNamed:@"save_button.png"];
			break;
		case SIAlertViewButtonTypeDestructive:
		default:
			normalImage = [UIImage imageNamed:@"next_button.png"];
			break;
	}
...
- (void)buttonAction:(UIButton *)button
{
    ...
    if (item.appAudio != AppNoAudio)
    {
        [self.appAudio play:item.appAudio withItem:item];
...
//            self.alpha = 0.0f;
    ...
    else
    {
        ...
        if (item.type != SIAlertViewButtonTypeSave)
        {
            [self dismissAnimated:YES];
        }
    }
