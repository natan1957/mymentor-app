...
//    CGFloat inset = self.bounds.size.height;
    self.imageView.frame = CGRectMake(0.f, 0.f, 72.f, 72.f);
...
//        _tintColor = [UIColor colorWithWhite:0.2 alpha:0.73];
        _tintColor = [UIColor colorWithRed:0.f green:67.f/255.f blue:99.f/255.f alpha:0.85f];
        _borderWidth = 2;
        _itemBackgroundColor = [UIColor clearColor];
//        [UIColor colorWithRed:1 green:1 blue:1 alpha:0.25];
    }
    return self;
}

- (instancetype)initWithImages:(NSArray *)images selectedImages:(NSArray *)selectedImages selectedIndex:(NSInteger)index
{
    if (self = [super init])
    {
        _isSingleSelect = NO;
        ...
        _width = 120;
        _animationDuration = 0.25f;
        _itemSize = CGSizeMake(72.f, 72.f);
        _itemViews = [NSMutableArray array];
//        _tintColor = [UIColor colorWithWhite:0.2 alpha:0.73];
        _tintColor = [UIColor colorWithRed:0.f green:67.f/255.f blue:99.f/255.f alpha:0.85f];
        _borderWidth = 2;
...
//        _borderColors = colors;
        ...
        [_images enumerateObjectsUsingBlock:^(UIImage *image, NSUInteger idx, BOOL *stop)
        {
            ...
            if (idx == index)
                view.imageView.image = self.selectedImages[idx];
            else
...
//            if (_borderColors && _selectedIndices)
//            {
...
{
    return [self initWithImages:images selectedImages:selectedImages selectedIndex:-1];
}

- (instancetype)initWithImages:(NSArray *)images
{
    return [self initWithImages:images selectedImages:nil selectedIndex:-1];
}
...
//        UIColor *stroke = self.borderColors[index];
        RNCalloutItemView *view = self.itemViews[index];
        self.selectedIndex = index;
...
//    }
    ...
    {
        [self.delegate sidebar:self didEnable:NO itemAtIndex:index];
    }
}
...
- (void)layoutItems
{
    ...
    [self.itemViews enumerateObjectsUsingBlock:^(RNCalloutItemView *view, NSUInteger idx, BOOL *stop)
    {
        CGRect frame = CGRectMake(leftPadding, 10.f*idx + self.itemSize.height *idx + topPadding, self.itemSize.width, self.itemSize.height);
        view.frame = frame;
        view.layer.cornerRadius = frame.size.width/2.f;
        if (idx == self.selectedIndex)
            view.imageView.image = self.selectedImages[idx];
        else
            view.imageView.image = self.images[idx];
    }];
