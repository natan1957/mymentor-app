...
@interface DTAttributedCustomTextView ()

@property (strong, nonatomic) NSMutableArray *lifo;
...
@implementation DTAttributedCustomTextView
{
...
- (void)_setup
{
	...
	self.lifo = [[NSMutableArray alloc] initWithCapacity:1];
	self.scrollTimer = [NSTimer scheduledTimerWithTimeInterval:0.1
														target:self
													  selector:@selector(checkLIFO)
													  userInfo:nil
													   repeats:YES];
}
...
{
	_lastOffsety = lastOffsety;
}

- (void)userFinishToDrag
{
	self.userDidScroll = YES;
	...
	NSDate *newDate = [[NSCalendar currentCalendar] dateByAddingComponents:dateComponents
																	toDate:futureDate
																   options:0];
	[self.lifo removeAllObjects];
	[self.lifo addObject:newDate];
}
...
- (void)checkLIFO
{
	...
	NSDate *previousDate = self.lifo[0];

	NSDateComponents *dateComponents1 = [[NSCalendar currentCalendar] components:NSHourCalendarUnit | NSMinuteCalendarUnit | NSSecondCalendarUnit fromDate:previousDate];
	NSDateComponents *dateComponents2 = [[NSCalendar currentCalendar] components:NSHourCalendarUnit | NSMinuteCalendarUnit | NSSecondCalendarUnit fromDate:[NSDate date]];

	if (dateComponents1.second == dateComponents2.second)
	{
		[self.lifo removeAllObjects];
		[self scrollRangeToVisible:self.lastRange
							 reset:NO];
	}
}

- (void)scrollToLastRange
{
	[self scrollRangeToVisible:self.lastRange
						 reset:YES];
}
...
- (void)scrollRangeToVisible:(NSRange)range reset:(BOOL)reset
{
	if (!reset)
	{
		if ([[NSDate date] timeIntervalSinceDate:self.lastScrollDate] < 3.f)
		{
			NSDate *futureDate = [NSDate date];
	...
	// get the line of the first index of the anchor range
	DTCoreTextLayoutLine *lineStart = [self.attributedTextContentView.layoutFrame lineContainingIndex:range.location];
	DTCoreTextLayoutLine *lineEnd = [self.attributedTextContentView.layoutFrame lineContainingIndex:(range.location+range.length)];

	CGFloat newOffsety = lineStart.frame.origin.y;
	CGFloat lineHeight = lineEnd.frame.origin.y - lineStart.frame.origin.y;

	if (newOffsety > self.lastOffsety)
	{
		if (newOffsety - self.lastOffsety < maxHeight)
		{
			if ((newOffsety + lineHeight) - self.lastOffsety > maxHeight)
			{
				if (newOffsety < 30.f)
				{
					newOffsety = 30.f;
				}
				self.userDidScroll = NO;
				[self setContentOffset:CGPointMake(0.f, newOffsety - 30.f) animated:YES];
				self.lastOffsety = newOffsety;
			}
			else
			{
				//				if (newOffsety < 30.f)
				...
				//				}
				//				self.userDidScroll = NO;
				//				[self setContentOffset:CGPointMake(0.f, newOffsety - 30.f) animated:animated];
				//				self.lastOffsety = newOffsety;
			}
		}
		else
	...
	{
		if (self.lastOffsety < 30.f)
		{
			self.userDidScroll = NO;
			[self setContentOffset:CGPointMake(0.f, 0.f) animated:YES];
		}
		else
		{
			if (newOffsety < 30.f)
...
- (void)contentViewDidLayout:(NSNotification *)notification
{
	DTBlockPerformSyncIfOnMainThreadElseAsync(^{
		...
		{
			self.attributedTextContentView.frame = optimalFrame;
			self.contentSize = [self.attributedTextContentView intrinsicContentSize];
		}
	});
}
...
//
//		// notification that tells us about the actual size of the content view
////		[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(contentViewDidLayout:) name:DTAttributedTextContentViewDidFinishLayoutNotification object:self.attributedTextContentView];
//
//		// temporary frame to specify the width
//		_attributedTextContentView.frame = frame;
//
